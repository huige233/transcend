...
public class ElementalWarden extends PhaseDrivenBossBase {
    ...
    @Override
    protected int getBaseSpellTier() { return 3; }
    ...
    protected void spawnAmbientParticles() {
        ...
        float r = currentElement.getParticleR(), g = currentElement.getParticleG(), b = currentElement.getParticleB();

        BossParticleModel.renderWardenBody(sl,
                this.getX(), this.getY(), this.getZ(), r, g, b, this.tickCount);

        Vec3 center = new Vec3(this.getX(), this.getY(), this.getZ());
        if (this.tickCount % 3 == 0) {
            ServerVisualBroadcaster.shieldRipple(sl, center.add(0.0, 1.15, 0.0),
                    1.25F + 0.18F * (float) Math.sin(this.tickCount * 0.05),
                    r, g, b, 14);
            ServerVisualBroadcaster.beam(sl, center.add(-1.2, 1.0, 0.0),
    ...
    private void deployMagicCircle(SpellElement element) {
        if (!(this.level() instanceof ServerLevel sl)) return;
        float r = element.getParticleR(), g = element.getParticleG(), b = element.getParticleB();
    ...
    protected java.util.List<net.minecraft.world.item.ItemStack> getBossDrops() {
        ...
        net.minecraft.world.item.ItemStack elementItem = switch (dropEl) {
            ...
            case WATER, WOOD -> new net.minecraft.world.item.ItemStack(ModItems.enhance_duration.get(), 2);
