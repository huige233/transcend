    ...
    public void tick() {
        ...
        if (this.level() instanceof ServerLevel sl) {
            ...
            if ((this.tickCount & 1) == 0) {
                ServerVisualBroadcaster.circle(sl, center, 0.95F, r, g, b, 12, 20, "pentagram");
            }
            if (this.tickCount % 3 == 0) {
                ServerVisualBroadcaster.shieldRipple(sl, center, 0.72F, r, g, b, 10);
            }
            if (this.tickCount % 4 == 0) {
                double bodyAngle = age * 0.15;
                Vec3 orb = center.add(Math.cos(bodyAngle) * 0.65, 0.15, Math.sin(bodyAngle) * 0.65);
                ServerVisualBroadcaster.beam(sl, orb, center, 0.95F, 0.75F, 0.25F, 10, "slash");
            }
        }
