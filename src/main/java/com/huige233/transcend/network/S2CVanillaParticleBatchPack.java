...
public class S2CVanillaParticleBatchPack {
    ...
    private static SimpleParticleType resolveParticle(String id) {
        String supportedId = resolveParticleId(id);
        if (supportedId == null) return null;
        return switch (supportedId) {
    ...
    static String resolveParticleId(String id) {
        if (id == null) return null;
        return switch (id) {
            case "enchant", "witch", "portal", "end_rod", "soul_fire_flame", "flame",
                    "snowflake", "smoke", "campfire_cosy_smoke", "ash" -> id;
            default -> null;
        };
    }
