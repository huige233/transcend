...
public class NodeDefinition {
    ...
    public void write(FriendlyByteBuf buf) {
        ...
        buf.writeInt(passiveEffects.size());
        for (PassiveEffect passive : passiveEffects) {
            buf.writeUtf(passive.toJson().toString());
        }
    ...
    public static NodeDefinition read(FriendlyByteBuf buf) {
        ...
        for (int i = 0; i < passiveCount; i++) {
            passives.add(PassiveEffect.fromJson(
                    com.google.gson.JsonParser.parseString(buf.readUtf()).getAsJsonObject()));
        }
        ...
        return new NodeDefinition(id, tier, cost, color, nameKey, descKey,
                parents, posX, posY, stats, passives, elementScaling);
    }
}
