...
public class BossHealthBarOverlay {
    ...
    private static void renderBar(GuiGraphics g, Minecraft mc, AbstractTranscendBoss boss, int sw, int y) {
        ...
        if (el != null) {
            ...
            int elColor = 0xFF000000 | ((int)(el.getParticleR() * 255) << 16)
                    | ((int)(el.getParticleG() * 255) << 8) | (int)(el.getParticleB() * 255);
    ...
    private static int[] getBarColor(AbstractTranscendBoss b, float p, float w1, float w2) {
        ...
        if (el != null) return new int[]{
                Mth.clamp((int)((el.getParticleR() * 200 + w1 * 55) * w2), 0, 255),
                Mth.clamp((int)((el.getParticleG() * 200 + w1 * 55) * w2), 0, 255),
                Mth.clamp((int)((el.getParticleB() * 200 + w1 * 55) * w2), 0, 255)};
