...
public abstract class AbstractMagicCircle {
    ...
    protected void submitShaderCircle(float scale) {
        ...
        ServerVisualBroadcaster.circle(level, center, radius, baseR, baseG, baseB,
    ...
    protected void sendBatch(List<S2CParticleBatchPack.ParticleEntry> entries,
        ...
        ServerVisualBroadcaster.particleBatch(level, center, entries, r, g, b, scale, lifetime, true);
    }
    ...
    protected void sendVanillaBatch(List<S2CVanillaParticleBatchPack.VanillaParticleEntry> entries, String particleId) {
        ...
        ServerVisualBroadcaster.vanillaParticleBatch(level, center, entries, particleId);
    }
    ...
    protected void sendRuneBatch(List<S2CRuneBatchPack.RuneEntry> entries,
        ...
        ServerVisualBroadcaster.runeBatch(level, center, entries, r, g, b, scale, lifetime, glow);
    }
    ...
    protected void sendGlitterBatch(List<S2CGlitterBatchPack.GlitterEntry> entries,
        ...
        ServerVisualBroadcaster.glitterBatch(level, center, entries, r, g, b, scale, lifetime, glow);
    }
    ...
    private void emitShaderFromDustBatch(List<S2CParticleBatchPack.ParticleEntry> entries,
        ...
        ServerVisualBroadcaster.circle(level, c, radius, r, g, b, life, 28, getCirclePattern());
        if (entries.size() > 24 && age % 8 == 0) {
            ServerVisualBroadcaster.shockwave(level, c, radius * 0.9F, r, g, b, Math.max(10, life - 4));
        }
    }
    ...
    private void emitShaderFromRuneBatch(List<S2CRuneBatchPack.RuneEntry> entries,
        ...
        for (int i = 0; i < limit; i++) {
            ...
            ServerVisualBroadcaster.beam(level, from, to, r, g, b,
                    Math.max(10, Math.min(30, lifetime + 8)), "beam");
        }
        if (age % 6 == 0) {
            S2CRuneBatchPack.RuneEntry first = entries.get(0);
            ServerVisualBroadcaster.shieldRipple(level, new Vec3(first.x, first.y, first.z),
                    Math.max(0.8F, scale * 1.5F), r, g, b, 18);
        }
    }
    ...
    private void emitShaderFromGlitterBatch(List<S2CGlitterBatchPack.GlitterEntry> entries,
        ...
        ServerVisualBroadcaster.shieldRipple(level, c, radius, r, g, b,
                Math.max(10, Math.min(32, lifetime + 6)));
    }
    ...
    private void emitShaderFromVanillaBatch(List<S2CVanillaParticleBatchPack.VanillaParticleEntry> entries,
        ...
        ServerVisualBroadcaster.shockwave(level, c, radius, r, g, b, 16);
    }
