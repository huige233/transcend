...
public class SoftGetHealthZeroCommand {
    public static ArgumentBuilder<CommandSourceStack, ?> register() {
        return Commands.literal("softGetHealthKill")
    ...
    private static int kill(CommandSourceStack p_137814_, Collection<? extends Entity> p_137815_) {
        for (Entity entity : p_137815_) {
            if (entity instanceof LivingEntity living)
                EntityASMUtil.setHealthDelta(living, Float.NEGATIVE_INFINITY);
            if (!(entity instanceof Player) && entity instanceof LivingEntity) {
                LivingEntityExpandedContext living = ((LivingEntityEC) entity).uom$livingECData();
                living.isDead = true;
            }
